(function (global, factory) {
  ...
  class RendererPool {
    ...
    getOrCreateTileRenderer() {
      ...
      var pool = this.map.get(Types.TILE);
      var found = null;
     ...
     */
    handleEngineReset() {
      var tilePool = this.map.get(Types.TILE);
      if (!tilePool) {
        return;
      }
  ...
  var rendererPool = new RendererPool();
  var RendererTypes = Types;
  ...
  class Engine {
     ...
     */
    createRenderer() {
      var renderer = rendererPool.get(RendererTypes.GAME);
    ...
    attachCamera(entity) {
      ...
      var prevEntity = this.cameras.get(camera);
      if (prevEntity) {
  ...
  class Camera {
     ...
     */
    getActiveCamera() {
      var cameras = [].concat( this.cameras.values() );
  ...
  class Controls extends Plugin {
    ...
    getRawControllerInput(controller) {
      ...
      if (this.hasController) {
        ...
        for (var key of Object.keys(input)) {
          ...
          var previousHadValue = this.previousInput[key] && this.previousInput[key] !== 0;
     ...
     */
    requestPointerLock() {
      var renderer = Engine.get().getRenderer();
      if (!renderer) {
        return;
      }
  ...
  class Models {
    ...
    async loadModel(directory, name, options) {
      ...
      // Remove lights.
      var light = root.getObjectByProperty('type', 'PointLight');
      if (light) {
  ...
  class Character extends Entity {
    ...
    sprint() {
      ...
      var action = this.playAnimation(this.sprintingAnimationName);
  ...
  class Light extends Plugin {
    ...
    createAmbientLight(ambientConfig) {
      var ambientLight = new THREE.AmbientLight(ambientConfig.color);
    ...
    createDirectionalLights(directionalConfig) {
      ...
      for (var i = 0; i < directionalConfig.length; i++) {
        var light = directionalConfig[i];
        var x = light.x;
        var y = light.y;
        var z = light.z;
        var color = parseInt(light.color, 16);
    ...
    createSpotLights(spotConfig) {
      ...
      for (var i = 0; i < spotConfig.length; i++) {
        ...
        var penumbra = light.penumbra;
        var shaders = light.shaders;
    ...
    loadLights(lightsData) {
      ...
      if (lightsData.ambient) {
        lightsData.ambient.forEach((data) => {
          var color = data.color !== undefined ?
      ...
      if (lightsData.directional) {
        lightsData.directional.forEach((data) => {
          ...
          var z = data.z ? data.z : 0;
          var intensity = data.intensity ? data.intensity : 1.0;
